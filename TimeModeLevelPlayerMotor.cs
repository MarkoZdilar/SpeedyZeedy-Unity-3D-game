...
public class TimeModeLevelPlayerMotor : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // Z
        TimeModeScore.metersLeft -= Time.deltaTime * 2;
        if((int)TimeModeScore.metersLeft == 0)
        {
            if(PlayerPrefs.GetFloat("HighscoreInSeconds") == 0.0)
                PlayerPrefs.SetFloat("HighscoreInSeconds", TimeModeScore.elapsedTime);
            else if (PlayerPrefs.GetFloat("HighscoreInSeconds") > TimeModeScore.elapsedTime)
                PlayerPrefs.SetFloat("HighscoreInSeconds", TimeModeScore.elapsedTime);
            Death();
            gameOver = true;
        }
        ...
        if (Input.GetKey(KeyCode.UpArrow))
        {
            ...
            TimeModeScore.metersLeft -= Time.deltaTime * 6;
        ...
        {
            movingVector.z = chSpeed / 3;
            TimeModeScore.metersLeft -= Time.deltaTime;
        }
        else
    ...
    private void Death()
    {
        ...
        FindObjectOfType<AudioManager>().Stop("Footsteps3");
        dead = true;
        GetComponent<TimeModeScore>().OnDeath();
    }
}
